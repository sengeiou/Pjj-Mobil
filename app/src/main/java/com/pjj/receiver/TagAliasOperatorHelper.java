...
public class TagAliasOperatorHelper {
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        String actionStr = getActionStr(tagAliasBean.action);
        if (jPushMessage.getErrorCode() == 0) {
            SharedUtils.saveForXml(SharedUtils.JPUSH_ALIAS, tagAliasBean.alias);
            Log.i(TAG, "action - modify alias Success,sequence:" + sequence);
            setActionCache.remove(sequence);
            String logs = actionStr + " alias success";
            Log.e(TAG, logs);
           // ExampleUtil.showToast(logs, context);
            //设置alias成功
            if (null != onAliasListener) {
                onAliasListener.result(true);
            }

        } else {
            //设置alias失败
            if ("set".equals(actionStr)) {
                if (null != onAliasListener) {
                    onAliasListener.result(false);
                }
            }

            String logs = "Failed to " + actionStr + " alias, errorCode:" + jPushMessage.getErrorCode();
            Log.e(TAG, logs);
            if (!RetryActionIfNeeded(jPushMessage.getErrorCode(), tagAliasBean)) {
                //ExampleUtil.showToast(logs, context);
            }
    ...
    private OnAliasListener onAliasListener;

    public void setOnAliasListener(OnAliasListener onAliasListener) {
        this.onAliasListener = onAliasListener;
    }
