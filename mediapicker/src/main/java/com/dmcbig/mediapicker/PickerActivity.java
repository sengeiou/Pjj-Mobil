...
public class PickerActivity extends Activity implements DataCallback, View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        category_btn.post(new Runnable() {
            @Override
            public void run() {
                Drawable drawable = ContextCompat.getDrawable(getApplication(), R.drawable.text_indicator);
                int top = (int) (category_btn.getHeight() / 4.5);
                int width = (int) (top * 1f / drawable.getIntrinsicHeight() * drawable.getIntrinsicWidth());
                //drawable.setBounds(0, top, width, category_btn.getHeight());
                drawable.setBounds(0, 0, width, top);
                category_btn.setCompoundDrawables(null, null, drawable, null);
            }
        });
