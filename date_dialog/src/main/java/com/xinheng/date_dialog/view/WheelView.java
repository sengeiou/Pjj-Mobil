package com.xinheng.date_dialog.view;
...
import com.xinheng.date_dialog.util.SizeConvertUtil;
...
public class WheelView extends View {
    ...
    private void init(Context context) {
        ...
        centerLinePaint.setColor(getResources().getColor(R.color.wheel_40bbf7));
...
//                canvas.drawRect(bottomRect, centerLinePaint);
        ...
        /*background=new Drawable() {
            ...
            public void draw(@NonNull Canvas canvas) {
                itemHeight = getMeasuredHeight() / (SHOW_SIZE * 2 + 1);
                int width = getMeasuredWidth();
                //float radius=width-getPaddingLeft()-getPaddingRight();
                float radius=itemHeight/2f+20;
                centerLinePaint.setColor(Color.GREEN);
                centerLinePaint.setStrokeWidth(2);
                centerLinePaint.setStyle(Style.STROKE);
                canvas.drawCircle(width/2f,getMeasuredHeight()/2f,radius,centerLinePaint);
            }
    ...
    private void performSelect() {
        if (mSelectListener != null) {
            if(itemList.size()>currentItem) {
                String text = itemList.get(currentItem);
                mSelectListener.onSelect(currentItem, text);
            }else {
                Log.e(TAG, "performSelect: "+"越界。。。" );
            }
        } else {
